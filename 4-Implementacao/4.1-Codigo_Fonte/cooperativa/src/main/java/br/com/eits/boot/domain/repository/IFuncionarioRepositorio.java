...
public interface IFuncionarioRepositorio extends JpaRepository<Funcionario, Long>
{
	...
	@Query(value="FROM Funcionario funcionario " +
				  "WHERE funcionario.ativo = true AND ( FILTER(funcionario.id, :filter) = TRUE "
				  	 + "OR FILTER(funcionario.nome, :filter) = TRUE OR FILTER(funcionario.cpf, :filter) = TRUE OR FILTER(funcionario.telefone, :filter) = TRUE OR FILTER(funcionario.celular, :filter) = TRUE)" )
